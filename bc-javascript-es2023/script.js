...
const deleteItem = id => {
  ...
  //
  // EJEMPLO:
  const filteredItems = items.filter(libro => libro.id !== id);
...
const toggleItemActive = id => {
  ...
  const updatedItems = items.map(libro =>
  libro.id === id
  ? { ...libro, active: !libro.active, updatedAt: new Date().toISOString() }
  : libro
  );
  saveItems(updatedItems);
  return updatedItems;
...
const activeItems = items.filter(libro => libro.active);
...
const filterByStatus = (itemsToFilter, status = 'all') => {
  ...
  if (status === 'active') return itemsToFilter.filter(libro => libro.active);
  if (status === 'inactive') return itemsToFilter.filter(libro => !libro.active);
...
const filterByCategory = (itemsToFilter, category = 'all') => {
  ...
  return itemsToFilter.filter(libro => libro.category === category);
};
...
const filterByPriority = (itemsToFilter, priority = 'all') => {
  ...
  return itemsToFilter.filter(libro => libro.priority === priority);
};
...
const searchItems = (itemsToFilter, query) => {
  ...
  return itemsToFilter.filter(libro =>
    libro.title.toLowerCase().includes(searchTerm) ||
    (libro.description ?? '').toLowerCase().includes(searchTerm)
  );
};
...
const getStats = (itemsToAnalyze = []) => {
  ...
  const active = itemsToAnalyze.filter(libro => libro.active).length;
  const inactive = total - active;
  //
  // // Usa reduce para agrupar por categoría
const byCategory = itemsToAnalyze.reduce((acc, libro) => {
acc[libro.category] = (acc[libro.category] ?? 0) + 1;
return acc;
}, {});
  //
  // // Usa reduce para agrupar por prioridad
const byPriority = itemsToAnalyze.reduce((acc, libro) => {
  acc[libro.priority] = (acc[libro.priority] ?? 0) + 1;
  return acc;
}, {});
...
const renderItem = item => {
  ...
  //
  // EJEMPLO:
  const { id, title,author, description, category, priority, active, createdAt } = item;
  ...
  <p class="item-author"><strong>Author:</strong> ${author}</p>
...
const handleFormSubmit = e => {
  ...
  const author = document.getElementById('item-author').value.trim() ?? 'Anoniimo';
  ...
  const itemData = { title, author, description, category, priority, price:Number(price)}; };
...
const handleItemEdit = itemId => {
  ...
  document.getElementById('item-author').value = itemToEdit.author;
  ...
  document.getElementById('submit-btn').textContent = 'Actualizar libro';
...
const handleItemDelete = itemId => {
  ...
  if (!confirm('¿Estás seguro de que deseas eliminar este libro del catalogo?')) return;
...
const resetForm = () => {
 ...
 document.getElementById('form-title').textContent = '➕ Nuevo libro';
  document.getElementById('submit-btn').textContent = 'Añadir al catalogo';
...
if (confirm('¿Deseas eliminar permanentemente todos los libros marcados como inactivos?')) {
items = clearInactive();
renderItems(applyCurrentFilters());
renderStats(getStats(items));
}
...
const init = () => {
  ...
  console.log('✅Catalogo de libros inicializado correctamente');
};
